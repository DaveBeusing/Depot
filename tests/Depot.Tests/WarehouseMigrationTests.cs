...
public sealed class WarehouseMigrationTests : IDisposable
{
	...
	public void VersionEightDataMigratesToWarehouseStructureWithoutLosingReferences()
	{
		...
		Assert.Equal(DatabaseVersion.CurrentVersion, ExecuteInt64(connection, "SELECT Version FROM DatabaseInfo;"));
		...
		Assert.Equal(10L, ExecuteInt64(connection, "SELECT COUNT(DISTINCT Code) FROM ReasonCodes WHERE IsSystem = 1;"));
