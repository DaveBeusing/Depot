...
internal sealed class ProcurementTestContext : IAsyncDisposable
{
	...
	private SupplierReturnService? _supplierReturns;
	...
	public SupplierReturnService SupplierReturns => _supplierReturns ?? throw new InvalidOperationException("The supplier return service was not initialized.");
	...
	private async Task RemoveTestRowsAsync()
	{
		...
		await Data.ExecuteAsync("DELETE FROM AuditEntries WHERE EntityType = 'SupplierReturn' AND EntityId IN (SELECT Id FROM SupplierReturns WHERE SupplierId IN ($First, $Second));", CancellationToken.None, new DatabaseParameter("$First", supplierIds[0]), new DatabaseParameter("$Second", supplierIds[1]));
		await Data.ExecuteAsync("DELETE FROM WorkflowOperations WHERE Workflow LIKE 'PurchaseOrder.%' AND EntityId IN (SELECT Id FROM PurchaseOrders WHERE SupplierId IN ($First, $Second));", CancellationToken.None, new DatabaseParameter("$First", supplierIds[0]), new DatabaseParameter("$Second", supplierIds[1]));
		await Data.ExecuteAsync("DELETE FROM WorkflowOperations WHERE Workflow = 'MaterialIssue.Post' AND EntityId IN (SELECT Id FROM MaterialIssues WHERE Recipient LIKE 'Material issue test %');", CancellationToken.None);
		await Data.ExecuteAsync("DELETE FROM WorkflowOperations WHERE Workflow = 'MaterialReturn.Post' AND EntityId IN (SELECT Id FROM MaterialReturns WHERE RecipientOrSource LIKE 'Material return test %');", CancellationToken.None);
		await Data.ExecuteAsync("DELETE FROM WorkflowOperations WHERE Workflow = 'SupplierReturn.Post' AND EntityId IN (SELECT Id FROM SupplierReturns WHERE SupplierId IN ($First, $Second));", CancellationToken.None, new DatabaseParameter("$First", supplierIds[0]), new DatabaseParameter("$Second", supplierIds[1]));
		...
		await Data.ExecuteAsync("DELETE FROM SupplierReturnLines WHERE SupplierReturnId IN (SELECT Id FROM SupplierReturns WHERE SupplierId IN ($First, $Second));", CancellationToken.None, new DatabaseParameter("$First", supplierIds[0]), new DatabaseParameter("$Second", supplierIds[1]));
		await Data.ExecuteAsync("DELETE FROM SupplierReturns WHERE SupplierId IN ($First, $Second);", CancellationToken.None, new DatabaseParameter("$First", supplierIds[0]), new DatabaseParameter("$Second", supplierIds[1]));
