...
public sealed class SalesWorkflowIntegrationTests : IAsyncLifetime
{
	...
	{
		var fixture = Fixture;
		var customer = await fixture.Customers.SaveAsync(new Customer
		{
		...
		var reserved = await fixture.Orders.SetReservationsAsync(approved.Id, approved.Version,
		[
			new SalesReservationRequest(approved.Lines[0].Id, fixture.InventoryId, 10)
		]);
		Assert.Equal(20, await fixture.CurrentStockAsync());
		...
		var shipment = await fixture.Shipments.CreateAsync(released.Id,
		[
			new ShipmentLineRequest(reservation.Id, 10)
		], "DHL", "TRACK-001", "Integration shipment");
	...
	public async Task PartialFulfillmentLeavesBackorderAndCanBeCompletedLater()
	{
		...
		{
			CustomerId = customer.Id,
			Lines = [new SalesOrderLine { ItemId = fixture.ItemId, Quantity = 15, UnitPrice = 10m, TaxRate = 19m }]
		});
		var submitted = await fixture.Orders.SubmitAsync(draft.Id, draft.Version);
		var approved = await fixture.Orders.ApproveAsync(submitted.Id, submitted.Version);
		var reserved = await fixture.Orders.SetReservationsAsync(approved.Id, approved.Version,
		[
			new SalesReservationRequest(approved.Lines[0].Id, fixture.InventoryId, 8)
		]);
		var released = await fixture.Orders.ReleaseAsync(reserved.Id, reserved.Version);
		...
		var reReserved = await fixture.Orders.SetReservationsAsync(partiallyShipped.Id, partiallyShipped.Version,
		[
			new SalesReservationRequest(partiallyShipped.Lines[0].Id, fixture.InventoryId, 7)
		]);
	...
	private sealed class SalesFixture
	{
		...
		public static async Task<SalesFixture> CreateAsync(IDatabaseConnectionFactory factory)
		{
			...
			var itemId = await data.InsertAsync("INSERT INTO Items (PartNumber,Description,IsActive) VALUES ($PartNumber,$Description,1);", CancellationToken.None,
				new DatabaseParameter("$PartNumber", $"SALES-{suffix}"), new DatabaseParameter("$Description", "Sales integration item"));
			...
			await data.InsertAsync("INSERT INTO StockMovements (InventoryId,MovementType,TimestampUtc,Quantity,UnitPrice,Reference) VALUES ($InventoryId,$Type,$Timestamp,20,0,$Reference);", CancellationToken.None,
				new DatabaseParameter("$InventoryId", inventoryId), new DatabaseParameter("$Type", (int)StockMovementType.OpeningBalance), new DatabaseParameter("$Timestamp", DateTime.UtcNow.ToString("O")), new DatabaseParameter("$Reference", "Sales integration opening stock"));
			...
			var customerReturns = new CustomerReturnService(runner, customerReturnRepository, shipmentRepository, movementRepository, auditRepository, audit, authorization);
			var credits = new SalesCreditNoteService(runner, creditNoteRepository, invoiceRepository, auditRepository, audit, authorization);
		...
		public async Task<Shipment> CreatePostedShipmentAsync(int quantity)
		{
			...
			var submitted = await Orders.SubmitAsync(draft.Id, draft.Version);
			var approved = await Orders.ApproveAsync(submitted.Id, submitted.Version);
			...
			var released = await Orders.ReleaseAsync(reserved.Id, reserved.Version);
			var reservation = Assert.Single(await Orders.GetReservationsAsync(released.Id), value => value.Status == InventoryReservationStatus.Active);
			var shipment = await Shipments.CreateAsync(released.Id, [new ShipmentLineRequest(reservation.Id, quantity)]);
			return await Shipments.PostAsync(shipment.Id, shipment.Version);
		}
	}
}
