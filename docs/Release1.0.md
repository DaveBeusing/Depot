...
Depot is on the `0.15.x-preview` line with Core database schema **30**, Sales feature schema **15**, Finance feature schema **12**, User Sessions schema **3**, Security Events schema **3**, User Preferences schema **2**, Document Templates schema **1**, Enterprise Identity feature schema **2**, and Help manifest **1.29**. `Directory.Build.props` is authoritative for the exact application patch/version. Checked items represent implemented technical controls/evidence only; they do not replace legal, accounting, accessibility, signing, localization or deployment acceptance.
