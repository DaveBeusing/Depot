...
Help manifest **1.24** contains Inventory, Warehouse, Purchasing, Sales, Approvals, Reports, Finance, Administration and Troubleshooting topics plus the current User Sessions and Security Center guidance.
...
These are article-content corrections only. Topic IDs, `requiredPermission` mappings, related-topic routing and manifest structure are unchanged, so Help manifest **1.24** remains the correct contract.
...
Help manifest **1.24** is the current documentation contract.
