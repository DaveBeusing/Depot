...
Depot's current `0.15.x-preview` line includes the integrated Finance platform, persistent session/security administration, production-tested database-provider support, advanced commercial price generation and the bounded XRechnung 3.0 CII electronic-invoice path.
...
The retained conformance fixtures for the advertised matrix are generated by the production `ElectronicInvoiceService` and externally validated through the pinned KoSIT/XRechnung configuration. ZUGFeRD/Factur-X remains outside the current product claim pending a separate product decision and PDF/A-3 implementation/evidence package.
