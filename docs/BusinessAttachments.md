...
The initial allowlist covers Customer, Supplier, Item, Sales Quote, Sales Order, Sales Invoice, Purchase Order, Goods Receipt and Supplier/AP Document. Arbitrary table names are not accepted. Service-layer validation verifies the allowlisted entity kind, referenced record and the corresponding domain permission.
...
The persisted contract is tracked independently as **Business Attachments feature schema 1** in `DepotFeatureVersions`.
