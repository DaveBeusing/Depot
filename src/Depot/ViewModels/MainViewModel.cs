...
public sealed class MainViewModel : BaseViewModel, IDisposable
{
	...
	private void AddDirect<TViewModel>(ApplicationPermission permission,string name,string icon,Func<TViewModel> createContent,Func<TViewModel,CancellationToken,Task> loadAsync,string helpTopicId) where TViewModel:BaseViewModel { if(!_authorization.HasPermission(permission))return; NavigationItems.Add(new ShellNavigationItem(name,icon,()=>createContent(),(viewModel,token)=>loadAsync((TViewModel)viewModel,token),helpTopicId)); }
	...
	private void SetSelectedNavigationItem(ShellNavigationItem item){if(_selectedNavigationItem==item)return;_selectedNavigationItem=item;OnPropertyChanged(nameof(SelectedNavigationItem));}
