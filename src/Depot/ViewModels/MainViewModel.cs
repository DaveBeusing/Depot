...
public sealed class MainViewModel : BaseViewModel, IDisposable
{
	...
	private readonly Lazy<ReplenishmentViewModel> _replenishment;
	...
	private readonly Lazy<SubscriptionBillingViewModel> _subscriptionBilling;
	...
	private readonly Lazy<ProductionViewModel>? _production;
	...
	public MainViewModel(
		...
		FinanceSepaPaymentExportService financeSepaPaymentExportService,
		FinanceFinancialReportingService financeFinancialReportingService,
		FinanceLocalizationService financeLocalizationService,
		...
		ProductionService? productionService = null)
	{
		...
		_replenishment = new(() => new ReplenishmentViewModel(replenishmentService));
		...
		_financeBanking = new(() => new FinanceBankingViewModel(financeBankingService, financeSepaPaymentExportService, fileDialogService));
		...
		if(productionService is not null)_production=new(()=>new ProductionViewModel(productionService,reasonCodeService));
	...
	public ReplenishmentViewModel ReplenishmentViewModel => _replenishment.Value;
	...
	public SubscriptionBillingViewModel SubscriptionBillingViewModel => _subscriptionBilling.Value;
	...
	public async Task OpenMyWorkItemAsync(MyWorkItem item, CancellationToken cancellationToken = default)
	{
		...
		switch (item.Kind)
		{
			...
			case MyWorkItemKind.ServiceOrder:
				...
				break;
			case MyWorkItemKind.ProductionOrder:
				await this.NavigateToRouteAsync(route, cancellationToken);
				if (_production is not null) await _production.Value.OpenOrderAsync(item.EntityId, cancellationToken);
				break;
			default:
				await this.NavigateToRouteAsync(route, cancellationToken);
				break;
	...
	private void BuildNavigation()
	{
		...
		if(_production is not null) AddDirect(ApplicationPermission.ProductionView, "Production", Icons.Warehouse, () => _production.Value, (viewModel, token) => viewModel.LoadAsync(token), "production.assembly");
	...
	public void Dispose()
	{
		...
		if(_procurement.IsValueCreated)_procurement.Value.Dispose();if(_procurementSourcing.IsValueCreated)_procurementSourcing.Value.Dispose();if(_replenishment.IsValueCreated)_replenishment.Value.Dispose();if(_salesOverview.IsValueCreated)_salesOverview.Value.Dispose();if(_salesQuotes.IsValueCreated)_salesQuotes.Value.Dispose();if(_salesPricing.IsValueCreated)_salesPricing.Value.Dispose();if(_salesCustomers.IsValueCreated)_salesCustomers.Value.Dispose();if(_salesOrders.IsValueCreated)_salesOrders.Value.Dispose();if(_salesApprovals.IsValueCreated)_salesApprovals.Value.Dispose();if(_salesShipping.IsValueCreated)_salesShipping.Value.Dispose();if(_salesInvoices.IsValueCreated)_salesInvoices.Value.Dispose();if(_subscriptionBilling.IsValueCreated)_subscriptionBilling.Value.Dispose();if(_salesSearch.IsValueCreated)_salesSearch.Value.Dispose();if(_financePostingFlowDesigner.IsValueCreated)_financePostingFlowDesigner.Value.Dispose();if(_financePeriodControl.IsValueCreated)_financePeriodControl.Value.Dispose();if(_financeReceivables.IsValueCreated)_financeReceivables.Value.Dispose();if(_financePayables.IsValueCreated)_financePayables.Value.Dispose();if(_financeInventoryAccounting.IsValueCreated)_financeInventoryAccounting.Value.Dispose();if(_financeBanking.IsValueCreated)_financeBanking.Value.Dispose();if(_financeFinancialReporting.IsValueCreated)_financeFinancialReporting.Value.Dispose();if(_production is not null&&_production.IsValueCreated)_production.Value.Dispose();if(_help.IsValueCreated){_help.Value.CloseRequested-=OnHelpCloseRequested;_help.Value.Dispose();}if(_notificationCenter.IsValueCreated){_notificationCenter.Value.CloseRequested-=OnNotificationCloseRequested;_notificationCenter.Value.Dispose();}
	}
