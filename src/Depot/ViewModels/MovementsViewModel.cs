...
public sealed class MovementsViewModel : BaseViewModel, IDisposable
{
	...
	public MovementsViewModel(MovementService movementService, ReasonCodeService reasonCodeService, IFileDialogService dialogs, Action? inventoryChanged = null)
	{
	...
	public IReadOnlyList<StockMovementType> MovementTypes { get; } = [StockMovementType.Purchase, StockMovementType.Withdrawal, StockMovementType.Correction];
	...
	public string TrackingFormatHint => TrackingAllocationTextParser.GenericFormatHint;

	public MovementOverviewItemViewModel? SelectedMovement { get => _selectedMovement; set { if (_selectedMovement == value) return; _selectedMovement = value; OnPropertyChanged(); ReverseMovementCommand.RaiseCanExecuteChanged(); } }
	public ReasonCodeOptionViewModel? SelectedReversalReasonCode { get => _selectedReversalReasonCode; set { if (_selectedReversalReasonCode == value) return; _selectedReversalReasonCode = value; OnPropertyChanged(); ReverseMovementCommand.RaiseCanExecuteChanged(); } }
	public string ReversalReason { get => _reversalReason; set { if (_reversalReason == value) return; _reversalReason = value; OnPropertyChanged(); ReverseMovementCommand.RaiseCanExecuteChanged(); } }

	public int PageNumber
	{
		get => _pageNumber;
		private set { if (_pageNumber == value) return; _pageNumber = value; OnPropertyChanged(); OnPropertyChanged(nameof(HasNextPage)); RaisePagingCommands(); }
	}

	public long TotalCount
	{
		get => _totalCount;
		private set { if (_totalCount == value) return; _totalCount = value; OnPropertyChanged(); OnPropertyChanged(nameof(HasNextPage)); RaisePagingCommands(); }
	}

	public string SearchText
	{
		get => _searchText;
		set { if (_searchText == value) return; _searchText = value; OnPropertyChanged(); PageNumber = 1; _ = _searchDebouncer.DebounceAsync(LoadMovementsAsync); }
	}

	public InventoryLookupViewModel? SelectedInventory
	{
		get => _selectedInventory;
		set { if (_selectedInventory == value) return; _selectedInventory = value; OnPropertyChanged(); Editor.InventoryId = value?.Id ?? 0; }
	}

	public string? ErrorMessage { get => _errorMessage; private set { if (_errorMessage == value) return; _errorMessage = value; OnPropertyChanged(); OnPropertyChanged(nameof(HasErrorMessage)); } }
	...
	public async Task LoadAsync(CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			{
				AvailableInventories.Add(new InventoryLookupViewModel { Id = inventory.Id, ItemId = inventory.ItemId, PartNumber = inventory.PartNumber, Description = inventory.Description, PurposeName = inventory.PurposeName, WarehouseName = inventory.WarehouseName, LocationName = inventory.LocationName });
			}
			ReasonCodes.Clear();
			ReasonCodes.Add(new ReasonCodeOptionViewModel(null, "No reason specified"));
			foreach (var reasonCode in await reasonCodesTask) ReasonCodes.Add(new ReasonCodeOptionViewModel(reasonCode.Id, reasonCode.Name));
			CompleteOperation(Items.Count == 0, $"{TotalCount:N0} movements");
		}
		catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { CompleteOperation(Items.Count == 0); }
		catch (Exception exception) { ErrorMessage = exception.Message; FailOperation(exception, "Movements could not be loaded"); }
	}
	...
	private async Task<bool> LoadMovementPageAsync(CancellationToken cancellationToken)
	{
		...
		try
		{
			var page = await _movementService.SearchAsync(SearchText, PageNumber, PageSize, request.Token);
		...
		catch (OperationCanceledException) when (request.Token.IsCancellationRequested) { return false; }
	}
	...
	private async Task CreateMovementAsync(CancellationToken cancellationToken)
	{
		...
		try
		{
			var tracking = TrackingAllocationTextParser.ParseUnspecified(Editor.TrackingText);
			var movement = Editor.MovementType switch
			{
				StockMovementType.Purchase => await _movementService.AddPurchaseAsync(Editor.InventoryId, Editor.Quantity, Editor.UnitPrice, Editor.ReasonCodeId, Editor.Reference, Editor.Notes, tracking, cancellationToken),
				StockMovementType.Withdrawal => await _movementService.AddWithdrawalAsync(Editor.InventoryId, Editor.Quantity, Editor.ReasonCodeId, Editor.Reference, Editor.Notes, tracking, cancellationToken),
				StockMovementType.Correction => await _movementService.AddCorrectionAsync(Editor.InventoryId, Editor.Quantity, Editor.ReasonCodeId, Editor.Reference, Editor.Notes, tracking, cancellationToken),
	...
	private async Task ReverseMovementAsync(CancellationToken cancellationToken)
	{
		...
		catch (Exception exception) when (exception is not OperationCanceledException) { FailOperation(exception, "Material withdrawal could not be reversed"); }
	}

	private async Task PreviousPageAsync(CancellationToken cancellationToken) { if (PageNumber <= 1) return; PageNumber--; await LoadMovementsAsync(cancellationToken); }
	private async Task NextPageAsync(CancellationToken cancellationToken) { if (!HasNextPage) return; PageNumber++; await LoadMovementsAsync(cancellationToken); }
