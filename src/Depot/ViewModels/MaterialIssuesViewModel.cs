...
public sealed class MaterialIssuesViewModel : BaseViewModel, IDisposable
{
	...
	private async Task PostAsync(CancellationToken cancellationToken) { if (!_dialogs.Confirm(new ConfirmationDialogRequest("Post Material Issue", $"Post {Draft.IssueNumber} and withdraw all line quantities?", true))) return; var operationId = Guid.NewGuid(); await ExecuteChangeAsync("Stocks are being checked and the material issue is posting", "Material issue posted", () => _service.PostMaterialIssueAsync(Draft.Id, Draft.Version, operationId, cancellationToken), MaterialIssueStatus.Posted, cancellationToken); }
	...
	private async Task ExecuteChangeAsync(string busy, string completed, Func<Task<MaterialIssue>> action, CancellationToken cancellationToken)
		=> await ExecuteChangeAsync(busy, completed, action, null, cancellationToken);
	private async Task ExecuteChangeAsync(string busy, string completed, Func<Task<MaterialIssue>> action, MaterialIssueStatus? expectedStatus, CancellationToken cancellationToken)
	{
		BeginOperation(busy);
		...
		catch (ConcurrencyConflictException exception) { if (expectedStatus is not null && await ReconcilePostAsync(completed)) return; FailOperation(exception, "The material issue was changed by another user"); }
		catch (Exception exception) when (exception is not OperationCanceledException) { if (expectedStatus is not null && await ReconcilePostAsync(completed)) return; FailOperation(exception, "Material issue operation failed"); }
	}
	private async Task<bool> ReconcilePostAsync(string completed)
	{
		try { var current = await _service.GetByIdAsync(Draft.Id, CancellationToken.None); if (current?.Status != MaterialIssueStatus.Posted) return false; var overview = await _service.GetOverviewByIdAsync(current.Id, CancellationToken.None); if (overview is not null) ApplyOverview(overview); Draft = Copy(current); Lines.Clear(); foreach (var line in current.Lines) Lines.Add(new MaterialIssueLineEditor(line)); Replace(Movements, await _service.GetMovementsAsync(current.Id, CancellationToken.None)); OnPropertyChanged(nameof(HasMovements)); CompleteOperation(false, $"{completed}; current server status confirmed"); return true; } catch { return false; }
	}
