...
public sealed class MaterialIssuesViewModel : BaseViewModel, IDisposable
{
	...
	private async Task PostAsync(CancellationToken cancellationToken) { if (!_dialogs.Confirm(new ConfirmationDialogRequest("Post Material Issue", $"Post {Draft.IssueNumber} and withdraw all line quantities?", true))) return; await ExecuteChangeAsync("Stocks are being checked and the material issue is posting", "Material issue posted", () => _service.PostAsync(Draft.Id, Draft.Version, cancellationToken), cancellationToken); }
