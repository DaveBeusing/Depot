...
public sealed class MaterialReturnsViewModel : BaseViewModel, IDisposable
{
	...
	public async Task OpenReturnAsync(long id, CancellationToken cancellationToken = default)
	{
		var overview = await _service.GetOverviewByIdAsync(id, cancellationToken)
			?? throw new InvalidOperationException("The referenced material return no longer exists.");
		var existing = Returns.FirstOrDefault(value => value.Id == id);
		if (existing is null)
		{
			Returns.Insert(0, overview);
			existing = overview;
		}
		SelectedReturn = existing;
	}
	...
	private async Task PostAsync(CancellationToken token) { if (!_dialogs.Confirm(new ConfirmationDialogRequest("Post Material Return", $"Post {Draft.ReturnNumber} and add all quantities to stock?", false))) return; var operationId = Guid.NewGuid(); await ExecuteChangeAsync("Material return is posting", "Material return posted", () => _service.PostMaterialReturnAsync(Draft.Id, Draft.Version, operationId, token), MaterialReturnStatus.Posted, token); }
	...
	private async Task ExecuteChangeAsync(string busy, string complete, Func<Task<MaterialReturn>> action, CancellationToken token) => await ExecuteChangeAsync(busy, complete, action, null, token);
	private async Task ExecuteChangeAsync(string busy, string complete, Func<Task<MaterialReturn>> action, MaterialReturnStatus? expectedStatus, CancellationToken token) { BeginOperation(busy); try { var changed = await action(); await ApplyCurrentAsync(changed, complete, token); } catch (ConcurrencyConflictException exception) { if (expectedStatus is not null && await ReconcilePostAsync(complete)) return; FailOperation(exception, "The material return was changed by another user"); } catch (Exception exception) when (exception is not OperationCanceledException) { if (expectedStatus is not null && await ReconcilePostAsync(complete)) return; FailOperation(exception, "Material return operation failed"); } }
	private async Task ApplyCurrentAsync(MaterialReturn changed, string complete, CancellationToken token) { var details = await _service.GetByIdAsync(changed.Id, token) ?? throw new InvalidOperationException("The material return was not found after the operation."); var overview = await _service.GetOverviewByIdAsync(changed.Id, token) ?? throw new InvalidOperationException("The material return overview was not found."); ApplyOverview(overview); Draft = Copy(details); Lines.Clear(); foreach (var line in details.Lines) Lines.Add(new MaterialReturnLineEditor(line)); Replace(Movements, await _service.GetMovementsAsync(changed.Id, token)); NotifyMovementState(); CompleteOperation(false, complete); }
	private async Task<bool> ReconcilePostAsync(string complete) { try { var current = await _service.GetByIdAsync(Draft.Id, CancellationToken.None); if (current?.Status != MaterialReturnStatus.Posted) return false; await ApplyCurrentAsync(current, $"{complete}; current server status confirmed", CancellationToken.None); return true; } catch { return false; } }
