...
public sealed class MaterialReturnsViewModel : BaseViewModel, IDisposable
{
	...
	private async Task PostAsync(CancellationToken token) { if (!_dialogs.Confirm(new ConfirmationDialogRequest("Post Material Return", $"Post {Draft.ReturnNumber} and add all quantities to stock?", false))) return; await ExecuteChangeAsync("Material return is posting", "Material return posted", () => _service.PostMaterialReturnAsync(Draft.Id, Draft.Version, token), token); }
