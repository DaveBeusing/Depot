...
public sealed class SalesInvoiceRepository : DatabaseRepository
{
	...
	public async Task<SalesInvoice?> GetByShipmentIdAsync(DatabaseTransactionContext tx,long shipmentId,CancellationToken token){var rows=await tx.Session.QueryAsync($"SELECT {Columns} {From} WHERE si.ShipmentId=$ShipmentId;",Read,token,Parameter("$ShipmentId",shipmentId));if(rows.Count==0)return null;var invoice=rows[0];invoice.Lines=await tx.Session.QueryAsync(LineSql+" WHERE sil.SalesInvoiceId=$Id ORDER BY sil.LineNumber;",ReadLine,token,Parameter("$Id",invoice.Id));return invoice;}
	public async Task<long> CreateAsync(DatabaseTransactionContext tx,SalesInvoice invoice,CancellationToken token){invoice.InvoiceNumber=$"PENDING-{Guid.NewGuid():N}";invoice.Id=await tx.Session.InsertAsync("INSERT INTO SalesInvoices (InvoiceNumber,CustomerId,SalesOrderId,ShipmentId,InvoiceDate,DueDate,Currency,Status,CustomerReference,BillingAddress,Notes,CreatedByUserId) VALUES ($Number,$CustomerId,$OrderId,$ShipmentId,$InvoiceDate,$DueDate,$Currency,$Status,$Reference,$Address,$Notes,$UserId);",token,Parameters(invoice));invoice.InvoiceNumber=$"INV-{invoice.Id:000000}";await tx.Session.ExecuteAsync("UPDATE SalesInvoices SET InvoiceNumber=$Number WHERE Id=$Id;",token,Parameter("$Number",invoice.InvoiceNumber),Parameter("$Id",invoice.Id));var lineNumber=1;foreach(var line in invoice.Lines){line.SalesInvoiceId=invoice.Id;line.LineNumber=lineNumber++;line.Id=await tx.Session.InsertAsync("INSERT INTO SalesInvoiceLines (SalesInvoiceId,LineNumber,SalesOrderLineId,ShipmentLineId,PartNumber,Description,Quantity,UnitPrice,DiscountPercent,TaxRate,TaxCategoryCode,TaxExemptionReasonCode,TaxExemptionReason) VALUES ($InvoiceId,$LineNumber,$OrderLineId,$ShipmentLineId,$PartNumber,$Description,$Quantity,$UnitPrice,$Discount,$TaxRate,$TaxCategory,$ExemptionCode,$ExemptionReason);",token,Parameter("$InvoiceId",invoice.Id),Parameter("$LineNumber",line.LineNumber),Parameter("$OrderLineId",line.SalesOrderLineId),Parameter("$ShipmentLineId",line.ShipmentLineId),Parameter("$PartNumber",line.PartNumber),Parameter("$Description",line.Description),Parameter("$Quantity",line.Quantity),Parameter("$UnitPrice",line.UnitPrice),Parameter("$Discount",line.DiscountPercent),Parameter("$TaxRate",line.TaxRate),Parameter("$TaxCategory",line.TaxCategoryCode),Parameter("$ExemptionCode",line.TaxExemptionReasonCode),Parameter("$ExemptionReason",line.TaxExemptionReason));}return invoice.Id;}
	public async Task<bool> UpdateTaxTreatmentAsync(DatabaseTransactionContext tx,long invoiceId,long lineId,string categoryCode,string? exemptionReasonCode,string? exemptionReason,CancellationToken token)=>await tx.Session.ExecuteAsync("UPDATE SalesInvoiceLines SET TaxCategoryCode=$Category,TaxExemptionReasonCode=$ReasonCode,TaxExemptionReason=$Reason,Version=Version+1 WHERE Id=$LineId AND SalesInvoiceId=$InvoiceId;",token,Parameter("$Category",categoryCode),Parameter("$ReasonCode",exemptionReasonCode),Parameter("$Reason",exemptionReason),Parameter("$LineId",lineId),Parameter("$InvoiceId",invoiceId))==1;
	...
	private const string LineSql="SELECT sil.Id,sil.SalesInvoiceId,sil.LineNumber,sil.SalesOrderLineId,sil.ShipmentLineId,sil.PartNumber,sil.Description,sil.Quantity,sil.UnitPrice,sil.DiscountPercent,sil.TaxRate,sil.TaxCategoryCode,sil.TaxExemptionReasonCode,sil.TaxExemptionReason,sil.Version FROM SalesInvoiceLines sil";
	...
	private static SalesInvoiceLine ReadLine(DbDataReader r)=>new(){Id=r.GetInt64(0),SalesInvoiceId=r.GetInt64(1),LineNumber=r.GetInt32(2),SalesOrderLineId=r.GetInt64(3),ShipmentLineId=r.GetInt64(4),PartNumber=r.GetString(5),Description=r.GetString(6),Quantity=r.GetInt32(7),UnitPrice=r.GetDecimal(8),DiscountPercent=r.GetDecimal(9),TaxRate=r.GetDecimal(10),TaxCategoryCode=r.GetString(11),TaxExemptionReasonCode=r.IsDBNull(12)?null:r.GetString(12),TaxExemptionReason=r.IsDBNull(13)?null:r.GetString(13),Version=r.GetInt64(14)};
}
