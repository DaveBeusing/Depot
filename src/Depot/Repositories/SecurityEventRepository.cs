...
public sealed class SecurityEventRepository : DatabaseRepository
{
	...
	public Task<SecurityCenterMetrics?> GetMetricsAsync(DateTime sinceUtc, CancellationToken cancellationToken) =>
		Database.QuerySingleOrDefaultAsync(
			...
			reader => new SecurityCenterMetrics(Convert.ToInt64(reader.GetValue(0), CultureInfo.InvariantCulture), Convert.ToInt64(reader.GetValue(1), CultureInfo.InvariantCulture), Convert.ToInt64(reader.GetValue(2), CultureInfo.InvariantCulture), Convert.ToInt64(reader.GetValue(3), CultureInfo.InvariantCulture)),
			cancellationToken, Parameter("$SinceUtc", Format(sinceUtc)));
	...
	public async Task<bool> MarkReviewedAsync(long id, long expectedVersion, long reviewedByUserId, DateTime reviewedUtc, CancellationToken cancellationToken) =>
		await Database.ExecuteAsync(
			...
			cancellationToken, Parameter("$ReviewedUtc", Format(reviewedUtc)), Parameter("$ReviewedByUserId", reviewedByUserId), Parameter("$Id", id), Parameter("$Version", expectedVersion)) == 1;

	private static DatabaseParameter[] Parameters(SecurityEvent securityEvent) =>
	[
		Parameter("$TimestampUtc", Format(securityEvent.TimestampUtc)), Parameter("$EventType", (int)securityEvent.EventType), Parameter("$Severity", (int)securityEvent.Severity),
		Parameter("$UserId", securityEvent.UserId), Parameter("$AccountIdentifier", securityEvent.AccountIdentifier),
		Parameter("$SessionId", securityEvent.SessionId?.ToString("D", CultureInfo.InvariantCulture)), Parameter("$ClientInstanceId", securityEvent.ClientInstanceId?.ToString("D", CultureInfo.InvariantCulture)),
	...
	{
		Id = reader.GetInt64(0), TimestampUtc = ReadUtc(reader, 1), EventType = (SecurityEventType)reader.GetInt32(2), Severity = (SecurityEventSeverity)reader.GetInt32(3),
		UserId = NullableInt64(reader, 4), AccountIdentifier = NullableString(reader, 5), SessionId = NullableGuid(reader, 6), ClientInstanceId = NullableGuid(reader, 7), MachineName = NullableString(reader, 8),
		Summary = reader.GetString(9), Details = NullableString(reader, 10), ReviewedUtc = ReadNullableUtc(reader, 11), ReviewedByUserId = NullableInt64(reader, 12), Version = reader.GetInt64(13)
	};
