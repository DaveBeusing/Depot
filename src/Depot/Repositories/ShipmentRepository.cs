...
{
	private const string Columns = "sh.Id,sh.ShipmentNumber,sh.SalesOrderId,so.OrderNumber,sh.CustomerId,c.Name,sh.ShipmentDate,sh.Status,sh.Carrier,sh.TrackingNumber,sh.ShippingAddress,sh.Notes,sh.CreatedByUserId,sh.PostedByUserId,sh.PostedAtUtc,sh.ReversedAtUtc,sh.ReversedByUserId,sh.ReversalReason,sh.Version";
	...
	public async Task<long> CreateAsync(DatabaseTransactionContext tx,Shipment shipment,CancellationToken token)
	{
		...
		shipment.Id=await tx.Session.InsertAsync("INSERT INTO Shipments (ShipmentNumber,SalesOrderId,CustomerId,ShipmentDate,Status,Carrier,TrackingNumber,ShippingAddress,Notes,CreatedByUserId) VALUES ($Number,$OrderId,$CustomerId,$Date,$Status,$Carrier,$Tracking,$Address,$Notes,$UserId);",token,Parameters(shipment));
	...
	private static Shipment Read(DbDataReader r)=>new(){Id=r.GetInt64(0),ShipmentNumber=r.GetString(1),SalesOrderId=r.GetInt64(2),SalesOrderNumber=r.GetString(3),CustomerId=r.GetInt64(4),CustomerName=r.GetString(5),ShipmentDate=Convert.ToDateTime(r.GetValue(6),CultureInfo.InvariantCulture),Status=(ShipmentStatus)r.GetInt32(7),Carrier=r.IsDBNull(8)?null:r.GetString(8),TrackingNumber=r.IsDBNull(9)?null:r.GetString(9),ShippingAddress=r.IsDBNull(10)?null:r.GetString(10),Notes=r.IsDBNull(11)?null:r.GetString(11),CreatedByUserId=r.GetInt64(12),PostedByUserId=r.IsDBNull(13)?null:r.GetInt64(13),PostedAtUtc=r.IsDBNull(14)?null:DateTime.Parse(r.GetString(14),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind),ReversedAtUtc=r.IsDBNull(15)?null:DateTime.Parse(r.GetString(15),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind),ReversedByUserId=r.IsDBNull(16)?null:r.GetInt64(16),ReversalReason=r.IsDBNull(17)?null:r.GetString(17),Version=r.GetInt64(18)};
