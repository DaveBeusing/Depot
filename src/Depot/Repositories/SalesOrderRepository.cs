...
public sealed class SalesOrderRepository : DatabaseRepository
{
	...
	public async Task<SalesOrder> SaveDraftAsync(DatabaseTransactionContext transaction, SalesOrder order, CancellationToken token)
	{
		...
		foreach (var line in order.Lines)
		{
			...
			if (line.Id == 0) line.Id = await session.InsertAsync("INSERT INTO SalesOrderLines (SalesOrderId,LineNumber,ItemId,PartNumber,Description,Quantity,UnitPrice,DiscountPercent,PriceSourceListId,PriceSourceName,PriceSourceScope,PriceSourceCurrency,TaxRate) VALUES ($SalesOrderId,$LineNumber,$ItemId,$PartNumber,$Description,$Quantity,$UnitPrice,$DiscountPercent,$PriceSourceListId,$PriceSourceName,$PriceSourceScope,$PriceSourceCurrency,$TaxRate);", token, LineParameters(line));
			else
			{
				var updated = await session.ExecuteAsync("UPDATE SalesOrderLines SET LineNumber=$LineNumber,ItemId=$ItemId,PartNumber=$PartNumber,Description=$Description,Quantity=$Quantity,UnitPrice=$UnitPrice,DiscountPercent=$DiscountPercent,PriceSourceListId=$PriceSourceListId,PriceSourceName=$PriceSourceName,PriceSourceScope=$PriceSourceScope,PriceSourceCurrency=$PriceSourceCurrency,TaxRate=$TaxRate,Version=Version+1 WHERE Id=$Id AND SalesOrderId=$SalesOrderId AND Version=$Version AND ReservedQuantity=0 AND ShippedQuantity=0;", token, [.. LineParameters(line), Parameter("$Id", line.Id), Parameter("$Version", line.Version)]);
	...
	[
		new("$SalesOrderId", l.SalesOrderId), new("$LineNumber", l.LineNumber), new("$ItemId", l.ItemId), new("$PartNumber", l.PartNumber), new("$Description", l.Description), new("$Quantity", l.Quantity), new("$UnitPrice", l.UnitPrice), new("$DiscountPercent", l.DiscountPercent), new("$PriceSourceListId", l.PriceSourceListId), new("$PriceSourceName", l.PriceSourceName), new("$PriceSourceScope", l.PriceSourceScope is null ? null : (int)l.PriceSourceScope.Value), new("$PriceSourceCurrency", l.PriceSourceCurrency), new("$TaxRate", l.TaxRate)
	];
	private const string LineSql = "SELECT sol.Id,sol.SalesOrderId,sol.LineNumber,sol.ItemId,sol.PartNumber,sol.Description,sol.Quantity,sol.UnitPrice,sol.DiscountPercent,sol.PriceSourceListId,sol.PriceSourceName,sol.PriceSourceScope,sol.PriceSourceCurrency,sol.TaxRate,sol.ReservedQuantity,sol.ShippedQuantity,sol.InvoicedQuantity,sol.Version FROM SalesOrderLines sol";
	...
	private static SalesOrderLine ReadLine(DbDataReader r) => new() { Id=r.GetInt64(0), SalesOrderId=r.GetInt64(1), LineNumber=r.GetInt32(2), ItemId=r.GetInt64(3), PartNumber=r.GetString(4), Description=r.GetString(5), Quantity=r.GetInt32(6), UnitPrice=r.GetDecimal(7), DiscountPercent=r.GetDecimal(8), PriceSourceListId=r.IsDBNull(9)?null:r.GetInt64(9), PriceSourceName=r.IsDBNull(10)?null:r.GetString(10), PriceSourceScope=r.IsDBNull(11)?null:(SalesPriceListScope)r.GetInt32(11), PriceSourceCurrency=r.IsDBNull(12)?null:r.GetString(12), TaxRate=r.GetDecimal(13), ReservedQuantity=r.GetInt32(14), ShippedQuantity=r.GetInt32(15), InvoicedQuantity=r.GetInt32(16), Version=r.GetInt64(17) };
