...
public sealed class SalesQuoteRepository : DatabaseRepository
{
	...
	public async Task<SalesQuote> SaveDraftAsync(SalesQuote quote,CancellationToken token)
	{
		...
		foreach(var line in quote.Lines)
		{
			...
			line.Id=await Database.InsertAsync("INSERT INTO SalesQuoteLines (SalesQuoteId,LineNumber,ItemId,PartNumber,Description,Quantity,UnitPrice,DiscountPercent,PriceSourceListId,PriceSourceName,PriceSourceScope,PriceSourceCurrency,TaxRate) VALUES ($QuoteId,$LineNumber,$ItemId,$PartNumber,$Description,$Quantity,$UnitPrice,$Discount,$PriceSourceListId,$PriceSourceName,$PriceSourceScope,$PriceSourceCurrency,$Tax);",token,
				Parameter("$QuoteId",quote.Id),Parameter("$LineNumber",line.LineNumber),Parameter("$ItemId",line.ItemId),Parameter("$PartNumber",line.PartNumber),Parameter("$Description",line.Description),Parameter("$Quantity",line.Quantity),Parameter("$UnitPrice",line.UnitPrice),Parameter("$Discount",line.DiscountPercent),Parameter("$PriceSourceListId",line.PriceSourceListId),Parameter("$PriceSourceName",line.PriceSourceName),Parameter("$PriceSourceScope",line.PriceSourceScope is null?null:(int)line.PriceSourceScope.Value),Parameter("$PriceSourceCurrency",line.PriceSourceCurrency),Parameter("$Tax",line.TaxRate));
		}
