...
public sealed class UserSessionRepository : DatabaseRepository
{
	...
	public Task<long> CreateAsync(UserSession session, CancellationToken cancellationToken) =>
		Database.InsertAsync(InsertSessionSql, cancellationToken, SessionInsertParameters(session));

	public static Task<long> CreateAsync(DatabaseTransactionContext transaction, UserSession session, CancellationToken cancellationToken) =>
		transaction.Session.InsertAsync(InsertSessionSql, cancellationToken, SessionInsertParameters(session));
	...
	public static Task<int> EndActiveSessionsForUserExceptAsync(DatabaseTransactionContext transaction, long userId, Guid? excludedSessionId, DateTime endedUtc, UserSessionEndReason reason, CancellationToken cancellationToken)
	{
		...
		return transaction.Session.ExecuteAsync(
			$"UPDATE UserSessions SET EndedUtc=$EndedUtc, EndReason=$EndReason, Version=Version+1 WHERE UserId=$UserId AND EndedUtc IS NULL{exclusion};",
			cancellationToken,
			parameters.ToArray());
	}
	...
	public static Task<int> AcquirePolicyLockAsync(DatabaseTransactionContext transaction, CancellationToken cancellationToken) =>
		transaction.Session.ExecuteAsync("UPDATE UserSessionPolicy SET Version=Version WHERE Id=1;", cancellationToken);
	...
	public Task<IReadOnlyList<ActiveUserSession>> GetActiveSessionsAsync(DateTime presenceCutoffUtc, string? searchText, CancellationToken cancellationToken)
	{
		...
		return Database.QueryAsync(
			$"SELECT s.Id,s.SessionId,s.UserId,u.Email,u.DisplayName,s.StartedUtc,s.LastSeenUtc,s.LastActivityUtc,s.ClientInstanceId,s.MachineName,s.AppVersion,s.Version FROM UserSessions s INNER JOIN Users u ON u.Id=s.UserId WHERE s.EndedUtc IS NULL AND s.LastSeenUtc >= $PresenceCutoff{filter} ORDER BY s.LastSeenUtc DESC,s.Id DESC;",
			ReadActiveSession,
			cancellationToken,
			parameters.ToArray());
	}
	...
	public Task<IReadOnlyList<EndedUserSession>> GetRecentEndedSessionsAsync(int count, CancellationToken cancellationToken)
	{
		...
		return Database.QuerySliceAsync(
			"SELECT s.Id,s.SessionId,s.UserId,u.Email,u.DisplayName,s.StartedUtc,s.LastSeenUtc,s.EndedUtc,s.EndReason,s.ClientInstanceId,s.MachineName,s.AppVersion,s.Version FROM UserSessions s INNER JOIN Users u ON u.Id=s.UserId WHERE s.EndedUtc IS NOT NULL ORDER BY s.EndedUtc DESC,s.Id DESC",
			ReadEndedSession,
			0,
			count,
			cancellationToken);
	...
	public Task<IReadOnlyList<long>> GetEndedSessionIdsBeforeAsync(DateTime cutoffUtc, int count, CancellationToken cancellationToken)
	{
		if (count is < 1 or > 1000) throw new ArgumentOutOfRangeException(nameof(count));
		return Database.QuerySliceAsync(
			"SELECT Id FROM UserSessions WHERE EndedUtc IS NOT NULL AND EndedUtc < $CutoffUtc ORDER BY EndedUtc, Id",
	...
	public static async Task<int> DeleteEndedSessionsByIdsBeforeAsync(DatabaseTransactionContext transaction, IReadOnlyCollection<long> ids, DateTime cutoffUtc, CancellationToken cancellationToken)
	{
		var deleted = 0;
		foreach (var id in ids)
		{
			deleted += await transaction.Session.ExecuteAsync(
				"DELETE FROM UserSessions WHERE Id=$Id AND EndedUtc IS NOT NULL AND EndedUtc < $CutoffUtc;",
				cancellationToken,
				Parameter("$Id", id),
				Parameter("$CutoffUtc", Format(cutoffUtc)));
		}
	...
	public async Task<UserSessionPolicy> GetPolicyAsync(CancellationToken cancellationToken) =>
		await Database.QuerySingleOrDefaultAsync($"SELECT {PolicyColumns} FROM UserSessionPolicy WHERE Id=1;", ReadPolicy, cancellationToken) ?? DefaultPolicy();
	...
	public static async Task<UserSessionPolicy> GetPolicyAsync(DatabaseTransactionContext transaction, CancellationToken cancellationToken) =>
		await transaction.Session.QuerySingleOrDefaultAsync($"SELECT {PolicyColumns} FROM UserSessionPolicy WHERE Id=1;", ReadPolicy, cancellationToken) ?? DefaultPolicy();
	...
	public async Task<bool> ExpireSessionIfPolicyExceededAsync(Guid sessionId, DateTime nowUtc, UserSessionPolicy policy, CancellationToken cancellationToken) =>
		await Database.ExecuteAsync(
			...
			Parameter("$MaximumAgeCutoff", Format(nowUtc - policy.MaximumSessionAge)),
	...
	private static DatabaseParameter[] SessionInsertParameters(UserSession session) =>
	[
	...
	[
		...
		Parameter("$ConcurrentSessionMode", (int)policy.ConcurrentSessionMode),
		Parameter("$MaximumConcurrentSessions", policy.MaximumConcurrentSessions),
		Parameter("$ConcurrentSessionLimitAction", (int)policy.ConcurrentSessionLimitAction),
		Parameter("$SessionHistoryRetentionDays", policy.SessionHistoryRetentionDays),
	...
	private static EndedUserSession ReadEndedSession(DbDataReader reader) => new(
		...
		ReadUtc(reader, 6),
		ReadUtc(reader, 7),
		(UserSessionEndReason)reader.GetInt32(8),
	...
	private static UserSessionPolicy ReadPolicy(DbDataReader reader) => new()
	{
		...
		ConcurrentSessionLimitAction = (ConcurrentSessionLimitAction)Convert.ToInt32(reader.GetValue(5), CultureInfo.InvariantCulture),
		SessionHistoryRetentionDays = Convert.ToInt32(reader.GetValue(6), CultureInfo.InvariantCulture),
		UpdatedUtc = ReadUtc(reader, 7),
