...
public sealed class DashboardRepository : DatabaseRepository
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (columns.Count == 0) return Task.FromResult<DashboardRoleMetrics?>(new DashboardRoleMetrics(null, null, null, null));

		return Database.QuerySingleOrDefaultAsync(
			$"SELECT {string.Join(", ", columns)};",
			reader => ReadMetrics(reader, includeApprovals, includePurchasing, includeWarehouse, includeAdministration),
			cancellationToken,
			parameters.ToArray());
	}

	private static DashboardRoleMetrics ReadMetrics(
		DbDataReader reader,
		bool includeApprovals,
		bool includePurchasing,
		bool includeWarehouse,
		bool includeAdministration)
	{
		...
		if (includeApprovals)
			approvals = new PurchaseOrderApprovalSummary(Long(reader, ordinal++), Utc(reader, ordinal++), Decimal(reader, ordinal++));
		if (includePurchasing)
			purchasing = new DashboardPurchasingMetrics(Long(reader, ordinal++), Long(reader, ordinal++), Long(reader, ordinal++), Long(reader, ordinal++));
		if (includeWarehouse)
			warehouse = new DashboardWarehouseMetrics(Long(reader, ordinal++), Long(reader, ordinal++));
		if (includeAdministration) administration = new DashboardAdministrationMetrics(Long(reader, ordinal));
		return new DashboardRoleMetrics(approvals, purchasing, warehouse, administration);
	}
	...
	private static DateTime? Utc(DbDataReader reader, int ordinal)
	{
		...
		var parsed = value is DateTime dateTime
			? dateTime
			: DateTime.Parse(Convert.ToString(value, CultureInfo.InvariantCulture) ?? string.Empty, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind);
