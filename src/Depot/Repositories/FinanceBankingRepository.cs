...
public sealed class FinanceBankingRepository : DatabaseRepository
{
	...
	internal Task<int> AdvancePaymentRunApprovalAsync(DatabaseTransactionContext transaction, long id, long version, CancellationToken cancellationToken) =>
		transaction.Session.ExecuteAsync("UPDATE FinancePaymentRuns SET Version=Version+1 WHERE Id=$Id AND Version=$Version AND Status=$Draft;", cancellationToken, Parameter("$Id", id), Parameter("$Version", version), Parameter("$Draft", (int)FinancePaymentRunStatus.Draft));
