...
public static class SecurityEventSchemaMigration
{
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version > CurrentVersion)
			throw new InvalidOperationException($"Security event schema version '{version}' is newer than the supported version '{CurrentVersion}'.");
		if (version == 0) MigrateToVersion1(connectionFactory);
		version = ReadVersion(connectionFactory);
		if (version != CurrentVersion)
			throw new InvalidOperationException($"Security event schema version '{version}' is not supported. Expected '{CurrentVersion}'.");
	}

	private static void MigrateToVersion1(IDatabaseConnectionFactory connectionFactory)
	{
		...
		var statements = connectionFactory.Provider switch
		{
			...
			_ => throw new NotSupportedException($"Security event schema is not supported for provider '{connectionFactory.Provider}'.")
		};
		...
		{
			DatabaseProvider.Local => $"INSERT INTO DepotFeatureVersions (Name, Version) VALUES ('{FeatureName}', 1) ON CONFLICT(Name) DO UPDATE SET Version=excluded.Version;",
			DatabaseProvider.SqlServer => $"IF EXISTS (SELECT 1 FROM DepotFeatureVersions WHERE Name=N'{FeatureName}') UPDATE DepotFeatureVersions SET Version=1 WHERE Name=N'{FeatureName}'; ELSE INSERT INTO DepotFeatureVersions (Name, Version) VALUES (N'{FeatureName}', 1);",
			DatabaseProvider.MySql => $"INSERT INTO DepotFeatureVersions (Name, Version) VALUES ('{FeatureName}', 1) ON DUPLICATE KEY UPDATE Version=VALUES(Version);",
