...
public static class SalesSchemaMigration
{
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version > CurrentVersion) throw new InvalidOperationException($"Sales schema version '{version}' is newer than the supported version '{CurrentVersion}'.");
		if (version == 0) { SalesSchemaInitializer.Ensure(connectionFactory); WriteVersion(connectionFactory, 1); version = 1; }
		if (version == 1) { SalesCorrectionSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 2); version = 2; }
		if (version == 2) { CustomerAddressSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 3); version = 3; }
		if (version == 3) { SalesReservationSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 4); version = 4; }
		if (version == 4) { SalesOrderAddressSnapshotSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 5); version = 5; }
		...
		if (version == 9) { ItemCostSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 10); version = 10; }
		if (version == 10) { SalesReservationSchema.EnsureActiveUniqueness(connectionFactory); WriteVersion(connectionFactory, 11); version = 11; }
		if (version == 11) { AdvancedPricingSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 12); version = 12; }
		if (version == 12) { ElectronicInvoiceCompletionSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 13); version = 13; }
		if (version == 13) { ZugferdFacturXSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 14); version = 14; }
		if (version == 14) { SalesCrmSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 15); version = 15; }
		if (version == 15) { SubscriptionBillingSchema.Ensure(connectionFactory); WriteVersion(connectionFactory, 16); version = 16; }
