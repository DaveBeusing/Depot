...
public static class ProcurementSourcingSchemaMigration
{
	public const int CurrentVersion = 1;
	private const string FeatureName = "ProcurementSourcing";
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version > CurrentVersion)
			throw new InvalidOperationException($"Procurement-sourcing schema version '{version}' is newer than the supported version '{CurrentVersion}'.");
		if (version == 0)
		{
			ProcurementSourcingSchema.Ensure(connectionFactory);
			WriteVersion(connectionFactory, CurrentVersion);
			version = CurrentVersion;
		}
		if (version != CurrentVersion)
			throw new InvalidOperationException($"Procurement-sourcing schema version '{version}' is not supported. Expected '{CurrentVersion}'.");
	}
	...
	private static void EnsureVersionTable(IDatabaseConnectionFactory connectionFactory)
	{
		...
		command.CommandText = connectionFactory.Provider switch
		{
			...
			_ => throw new NotSupportedException($"Procurement-sourcing migrations are not supported for provider '{connectionFactory.Provider}'.")
		};
