...
public static class EnterpriseIdentitySchemaMigration
{
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version > CurrentVersion)
			throw new InvalidOperationException($"Enterprise identity schema version '{version}' is newer than the supported version '{CurrentVersion}'.");
		...
		if (version != CurrentVersion)
			throw new InvalidOperationException($"Enterprise identity schema version '{version}' is not supported. Expected '{CurrentVersion}'.");
	}
	...
	private static void EnsureVersionTable(IDatabaseConnectionFactory connectionFactory)
	{
		...
		command.CommandText = connectionFactory.Provider switch
		{
			...
			_ => throw new NotSupportedException($"Enterprise identity migrations are not supported for provider '{connectionFactory.Provider}'.")
		};
