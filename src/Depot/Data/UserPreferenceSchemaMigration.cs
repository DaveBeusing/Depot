...
public static class UserPreferenceSchemaMigration
{
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version != CurrentVersion)
			throw new InvalidOperationException($"User preference schema version '{version}' is not supported. Expected '{CurrentVersion}'.");
	}
	...
	private static void EnsureVersionTable(IDatabaseConnectionFactory connectionFactory)
	{
		...
		command.CommandText = connectionFactory.Provider switch
		{
			...
			_ => throw new NotSupportedException($"User preference migrations are not supported for provider '{connectionFactory.Provider}'.")
		};
