...
public static class InventoryReplenishmentSchemaMigration
{
	public const int CurrentVersion = 1;
	private const string FeatureName = "InventoryReplenishment";
	...
	public static void Migrate(IDatabaseConnectionFactory connectionFactory)
	{
		...
		if (version > CurrentVersion)
			throw new InvalidOperationException($"Inventory-replenishment schema version '{version}' is newer than the supported version '{CurrentVersion}'.");
		if (version == 0)
		{
			InventoryReplenishmentSchema.Ensure(connectionFactory);
			WriteVersion(connectionFactory, CurrentVersion);
			version = CurrentVersion;
		}
		if (version != CurrentVersion)
			throw new InvalidOperationException($"Inventory-replenishment schema version '{version}' is not supported. Expected '{CurrentVersion}'.");
	}
	...
	private static void EnsureVersionTable(IDatabaseConnectionFactory connectionFactory)
	{
		...
		command.CommandText = connectionFactory.Provider switch
		{
			...
			_ => throw new NotSupportedException($"Inventory-replenishment migrations are not supported for provider '{connectionFactory.Provider}'.")
		};
