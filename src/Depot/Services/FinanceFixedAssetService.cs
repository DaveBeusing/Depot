...
public sealed class FinanceFixedAssetService
{
	...
	public async Task<FinanceAssetTransaction> PostDepreciationAsync(long schedulePeriodId,Guid operationId,FinanceClosedPeriodPolicy closedPeriodPolicy=FinanceClosedPeriodPolicy.Fail,CancellationToken token=default)
	{
		...
		return await _transactions.ExecuteAsync(async(tx,ct)=>{
			...
			await _assets.MarkSchedulePostedAsync(tx,schedule.Id,amount,journal.Id,operationId,ct);var tr=NewTransaction(asset.Id,FinanceAssetTransactionKind.Depreciation,operationId,target.PostingDate,amount,journal.Id,closedPeriodPolicy==FinanceClosedPeriodPolicy.NextOpenPeriod?"Explicit next-open-period policy":null,$"GL {journal.EntryNumber}",user.Id);var id=await _assets.CreateTransactionAsync(tx,tr,ct);tr=tr with{Id=id};await _auditEntries.CreateAsync(tx,_audit.CreateActionEntry(asset.Id,"DepreciationPosted",asset,asset),ct);return tr;
		},token);
	}
	...
	private async Task RequireProfileAsync(DatabaseTransactionContext tx,long profileId,Guid entity,string eventName,CancellationToken ct){if(profileId<=0)throw new InvalidOperationException($"A {eventName} posting profile is required.");var p=await _ledger.GetPostingProfileInTransactionAsync(tx,profileId,ct)??throw new InvalidOperationException($"{eventName} posting profile was not found.");if(!p.IsActive||p.LegalEntityId!=entity||!string.Equals(p.SourceType,SourceType,StringComparison.Ordinal)||!string.Equals(p.SourceEvent,eventName,StringComparison.Ordinal))throw new InvalidOperationException($"{eventName} posting profile must be active and use source {SourceType}/{eventName} for the asset legal entity.");}
