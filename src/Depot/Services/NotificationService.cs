...
public sealed class NotificationService : INotificationService
{
	...
	public Task<long> NotifyUserAsync(NotificationRequest request, long userId, CancellationToken cancellationToken = default) =>
		NotifyUsersAsync(request, [userId], cancellationToken);
	...
	public async Task<long> NotifyUsersAsync(NotificationRequest request, IEnumerable<long> userIds, CancellationToken cancellationToken = default)
	{
		...
		var id = await _transactions.ExecuteAsync(
			(transaction, token) => CreateAsync(transaction, request, recipients, token),
			cancellationToken);
		RaiseChanged();
		return id;
	}

	public async Task<long> NotifyPermissionHoldersAsync(
		NotificationRequest request,
		ApplicationPermission permission,
		IEnumerable<long>? excludedUserIds = null,
	...
	public async Task<long> NotifyAdministratorsAsync(NotificationRequest request, CancellationToken cancellationToken = default)
	{
		...
		var id = await _transactions.ExecuteAsync(
			async (transaction, token) =>
			{
				var users = await ResolveAdministratorsAsync(transaction, token);
				return await CreateAsync(transaction, request, users, token);
			}, cancellationToken);
		RaiseChanged();
		return id;
	}
	...
	public Task<long> GetUnreadCountAsync(CancellationToken cancellationToken = default) =>
		_notifications.GetUnreadCountAsync(CurrentUserId(), DateTime.UtcNow, cancellationToken);

	public Task<PageResult<NotificationListItem>> GetPageAsync(
		NotificationFilter filter,
		int pageNumber,
		int pageSize,
		CancellationToken cancellationToken = default) =>
		_notifications.GetPageAsync(CurrentUserId(), filter, pageNumber, pageSize, DateTime.UtcNow, cancellationToken);
	...
	public Task<IReadOnlyList<long>> ResolvePermissionHoldersAsync(
		DatabaseTransactionContext transaction,
		ApplicationPermission permission,
		CancellationToken cancellationToken) =>
		_notifications.GetActiveUserIdsWithPermissionAsync(transaction, permission, cancellationToken);

	public Task<IReadOnlyList<long>> ResolveAdministratorsAsync(
		DatabaseTransactionContext transaction,
		CancellationToken cancellationToken) =>
		_notifications.GetActiveAdministratorIdsAsync(transaction, cancellationToken);

	private async Task SetReadStateAsync(long recipientId, long version, DateTime? value, CancellationToken cancellationToken)
	{
		if (!await _notifications.SetReadStateAsync(recipientId, CurrentUserId(), version, value, cancellationToken))
			throw new ConcurrencyConflictException("notification");
	}
	...
	{
		if (!await _notifications.SetArchivedStateAsync(recipientId, CurrentUserId(), version, value, cancellationToken))
			throw new ConcurrencyConflictException("notification");
	}

	private long CurrentUserId() => _authorization.CurrentUser is { IsActive: true } user
		? user.Id
		: throw new UnauthorizedAccessException("An active signed-in user is required to access notifications.");
...
public sealed class NotificationNavigationService : INotificationNavigationService
{
	...
	public NotificationNavigationService(IAuthorizationService authorization)
	{
		_authorization = authorization;
	}
	...
	public Task NavigateAsync(NotificationDetails notification, CancellationToken cancellationToken = default)
	{
		if (string.IsNullOrWhiteSpace(notification.SourceType))
			throw new InvalidOperationException("This notification has no related Depot record.");
