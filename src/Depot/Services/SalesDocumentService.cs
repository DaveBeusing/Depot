...
public sealed class SalesDocumentService
{
	...
	public void CreateOrderConfirmation(string path, SalesOrder order)
	{
		...
		var document = CreateDocument("Order Confirmation", order.OrderNumber);
		var page = document.AddPage();
		var graphics = XGraphics.FromPdfPage(page);
		var y = DrawHeader(graphics, "ORDER CONFIRMATION", order.OrderNumber, order.CustomerName, order.OrderDate);
		y = DrawAddress(graphics, y, "Customer", order.CustomerName, null);
		y = DrawMetadata(graphics, y, [
			("Customer reference", order.CustomerReference ?? "—"),
			("Requested delivery", order.RequestedDeliveryDate?.ToString("d") ?? "—"),
			("Status", order.Status.ToString()),
			("Currency", order.Currency)
		]);
		y = DrawSalesLines(graphics, page, y, order.Lines, order.Currency, true);
		DrawTotals(graphics, page, y, order.NetAmount, order.TaxAmount, order.GrossAmount, order.Currency);
		Save(document, path);
	}
	...
	public void CreateDeliveryNote(string path, Shipment shipment)
	{
		...
		var document = CreateDocument("Delivery Note", shipment.ShipmentNumber);
		var page = document.AddPage();
		var graphics = XGraphics.FromPdfPage(page);
		var y = DrawHeader(graphics, "DELIVERY NOTE", shipment.ShipmentNumber, shipment.CustomerName, shipment.ShipmentDate);
		...
		DrawShipmentLines(graphics, page, y, shipment.Lines);
		Save(document, path);
	}
	...
	public void CreateInvoice(string path, SalesInvoice invoice)
	{
		...
		var document = CreateDocument("Sales Invoice", invoice.InvoiceNumber);
		var page = document.AddPage();
		var graphics = XGraphics.FromPdfPage(page);
		var y = DrawHeader(graphics, "INVOICE", invoice.InvoiceNumber, invoice.CustomerName, invoice.InvoiceDate);
		...
		y = DrawMetadata(graphics, y, [
			...
			("Due date", invoice.DueDate.ToString("d"))
		]);
		y = DrawInvoiceLines(graphics, page, y, invoice.Lines, invoice.Currency);
		DrawTotals(graphics, page, y, invoice.NetAmount, invoice.TaxAmount, invoice.GrossAmount, invoice.Currency);
		Save(document, path);
	}
	...
	private static PdfDocument CreateDocument(string title, string subject)
	{
		...
		document.Info.Title = $"Depot - {title}";
		document.Info.Subject = subject;
		document.Info.Creator = "Depot";
		return document;
	}

	private static double DrawHeader(XGraphics graphics, string title, string number, string customer, DateTime date)
	{
		graphics.DrawString("DEPOT", HeadingFont, XBrushes.Black, new XPoint(40, 45));
		graphics.DrawString(title, TitleFont, XBrushes.Black, new XPoint(40, 78));
		graphics.DrawString(number, HeadingFont, XBrushes.Black, new XPoint(40, 102));
		graphics.DrawString(date.ToString("d"), BodyFont, XBrushes.Black, new XPoint(470, 45));
		graphics.DrawString(customer, BodyFont, XBrushes.Black, new XPoint(360, 102));
		graphics.DrawLine(XPens.LightGray, 40, 116, 555, 116);
		return 142;
	}

	private static double DrawAddress(XGraphics graphics, double y, string heading, string customer, string? address)
	{
		graphics.DrawString(heading, HeadingFont, XBrushes.Black, new XPoint(40, y));
		y += 18;
		graphics.DrawString(customer, BodyFont, XBrushes.Black, new XPoint(40, y));
		y += 14;
		foreach (var line in (address ?? string.Empty).Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
		{
	...
	private static double DrawMetadata(XGraphics graphics, double y, IReadOnlyList<(string Label, string Value)> values)
	{
		var x = 40d;
		foreach (var value in values)
		{
			graphics.DrawString(value.Label, SmallFont, XBrushes.Gray, new XPoint(x, y));
			graphics.DrawString(value.Value, BodyFont, XBrushes.Black, new XPoint(x, y + 14));
			x += 128;
		}
		return y + 42;
	}

	private static double DrawSalesLines(XGraphics graphics, PdfPage page, double y, IReadOnlyList<SalesOrderLine> lines, string currency, bool showTax)
	{
		DrawTableHeader(graphics, y, "Item", "Description", "Qty", "Unit", showTax ? "Tax" : string.Empty, "Total");
		y += 22;
		foreach (var line in lines)
		{
			...
			graphics.DrawString(Trim(line.Description, 40), BodyFont, XBrushes.Black, new XPoint(125, y));
			...
			if (showTax) graphics.DrawString($"{line.TaxRate:N0}%", BodyFont, XBrushes.Black, new XPoint(475, y));
	...
	private static double DrawInvoiceLines(XGraphics graphics, PdfPage page, double y, IReadOnlyList<SalesInvoiceLine> lines, string currency)
	{
		DrawTableHeader(graphics, y, "Item", "Description", "Qty", "Unit", "Tax", "Total");
		y += 22;
		foreach (var line in lines)
		{
			y = EnsurePageSpace(ref graphics, page.Owner, ref page, y, 32);
	...
	private static void DrawShipmentLines(XGraphics graphics, PdfPage page, double y, IReadOnlyList<ShipmentLine> lines)
	{
		...
		graphics.DrawString("Quantity", HeadingFont, XBrushes.Black, new XPoint(480, y));
		y += 22;
		foreach (var line in lines)
		{
	...
	private static void DrawTableHeader(XGraphics graphics, double y, string c1, string c2, string c3, string c4, string c5, string c6)
	{
		graphics.DrawLine(XPens.LightGray, 40, y + 5, 555, y + 5);
		graphics.DrawString(c1, HeadingFont, XBrushes.Black, new XPoint(40, y));
		graphics.DrawString(c2, HeadingFont, XBrushes.Black, new XPoint(125, y));
		graphics.DrawString(c3, HeadingFont, XBrushes.Black, new XPoint(365, y));
		graphics.DrawString(c4, HeadingFont, XBrushes.Black, new XPoint(405, y));
		graphics.DrawString(c5, HeadingFont, XBrushes.Black, new XPoint(475, y));
		graphics.DrawString(c6, HeadingFont, XBrushes.Black, new XPoint(515, y));
	}

	private static void DrawTotals(XGraphics graphics, PdfPage page, double y, decimal net, decimal tax, decimal gross, string currency)
	{
		y = Math.Min(y + 10, page.Height.Point - 90);
		graphics.DrawLine(XPens.LightGray, 370, y, 555, y);
		graphics.DrawString("Net", BodyFont, XBrushes.Black, new XPoint(410, y + 18));
		graphics.DrawString(Money(net, currency), BodyFont, XBrushes.Black, new XPoint(500, y + 18));
		graphics.DrawString("Tax", BodyFont, XBrushes.Black, new XPoint(410, y + 34));
		graphics.DrawString(Money(tax, currency), BodyFont, XBrushes.Black, new XPoint(500, y + 34));
		graphics.DrawString("Total", HeadingFont, XBrushes.Black, new XPoint(410, y + 54));
		graphics.DrawString(Money(gross, currency), HeadingFont, XBrushes.Black, new XPoint(500, y + 54));
	}

	private static double EnsurePageSpace(ref XGraphics graphics, PdfDocument document, ref PdfPage page, double y, double required)
	{
		if (y + required < page.Height.Point - 55) return y;
		graphics.Dispose();
