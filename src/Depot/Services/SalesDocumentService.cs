...
public sealed class SalesDocumentService
{
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path); ArgumentNullException.ThrowIfNull(quote);
		var document=CreateDocument("Sales Quote",quote.QuoteNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		var y=DrawHeader(graphics,"QUOTE",quote.QuoteNumber,quote.CustomerName,quote.QuoteDate);
		...
		y=DrawQuoteLines(graphics,page,y,quote.Lines,quote.Currency);DrawTotals(graphics,page,y,quote.NetAmount,quote.TaxAmount,quote.GrossAmount,quote.Currency);Save(document,path);
	}

	public void CreateOrderConfirmation(string path, SalesOrder order)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(order);
		var document=CreateDocument("Order Confirmation",order.OrderNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		...
		y=DrawSalesLines(graphics,page,y,order.Lines,order.Currency,true);DrawTotals(graphics,page,y,order.NetAmount,order.TaxAmount,order.GrossAmount,order.Currency);Save(document,path);
	}

	public void CreatePickList(string path, Shipment shipment)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(shipment);
		var document=CreateDocument("Pick List",shipment.ShipmentNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		var y=DrawHeader(graphics,"PICK LIST",shipment.ShipmentNumber,shipment.CustomerName,shipment.ShipmentDate);
		...
		DrawShipmentLines(graphics,page,y,shipment.Lines);Save(document,path);
	}

	public void CreatePackingSlip(string path, Shipment shipment)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(shipment);
		var document=CreateDocument("Packing Slip",shipment.ShipmentNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		var y=DrawHeader(graphics,"PACKING SLIP",shipment.ShipmentNumber,shipment.CustomerName,shipment.ShipmentDate);
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(shipment);
		var document=CreateDocument("Delivery Note",shipment.ShipmentNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(invoice);
		var document=CreateDocument("Sales Invoice",invoice.InvoiceNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		...
		y=DrawInvoiceLines(graphics,page,y,invoice.Lines,invoice.Currency);DrawTotals(graphics,page,y,invoice.NetAmount,invoice.TaxAmount,invoice.GrossAmount,invoice.Currency);Save(document,path);
	}

	public void CreateCreditNote(string path, SalesCreditNote creditNote, SalesInvoice invoice)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(creditNote);ArgumentNullException.ThrowIfNull(invoice);
		var document=CreateDocument("Credit Note",creditNote.CreditNoteNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		...
		y=DrawCreditLines(graphics,page,y,creditNote.Lines,invoice.Lines,invoice.Currency);DrawTotals(graphics,page,y,-creditNote.NetAmount,-creditNote.TaxAmount,-creditNote.GrossAmount,invoice.Currency);Save(document,path);
	}

	public void CreateCustomerReturnReceipt(string path, CustomerReturn customerReturn, Shipment shipment)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path);ArgumentNullException.ThrowIfNull(customerReturn);ArgumentNullException.ThrowIfNull(shipment);
		var document=CreateDocument("Customer Return",customerReturn.ReturnNumber);var page=document.AddPage();var graphics=XGraphics.FromPdfPage(page);
		...
		DrawReturnLines(graphics,page,y,customerReturn.Lines,shipment.Lines);Save(document,path);
	}

	private static PdfDocument CreateDocument(string title,string subject){var document=new PdfDocument();document.Info.Title=$"Depot - {title}";document.Info.Subject=subject;document.Info.Creator="Depot";return document;}
	private static double DrawHeader(XGraphics graphics,string title,string number,string customer,DateTime date){graphics.DrawString("DEPOT",HeadingFont,XBrushes.Black,new XPoint(40,45));graphics.DrawString(title,TitleFont,XBrushes.Black,new XPoint(40,78));graphics.DrawString(number,HeadingFont,XBrushes.Black,new XPoint(40,102));graphics.DrawString(date.ToString("d"),BodyFont,XBrushes.Black,new XPoint(470,45));graphics.DrawString(customer,BodyFont,XBrushes.Black,new XPoint(360,102));graphics.DrawLine(XPens.LightGray,40,116,555,116);return 142;}
	...
	private static double EnsurePageSpace(ref XGraphics graphics,PdfDocument document,ref PdfPage page,double y,double required){if(y+required<page.Height.Point-55)return y;graphics.Dispose();page=document.AddPage();graphics=XGraphics.FromPdfPage(page);return 55;}
