...
public sealed class SalesDocumentService
{
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path); ArgumentNullException.ThrowIfNull(invoice); var issuer = ResolveInvoiceIssuer(invoice);
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(path); ArgumentNullException.ThrowIfNull(creditNote); ArgumentNullException.ThrowIfNull(invoice); var issuer = ResolveCreditNoteIssuer(creditNote);
	...
	private DocumentIssuerProfile ResolveInvoiceIssuer(SalesInvoice invoice) => invoice.Status == SalesInvoiceStatus.Posted
		? _issuerSnapshots.LoadRequired(DocumentIssuerSnapshotType.SalesInvoice, invoice.Id)
		: _issuerService.Load();

	private DocumentIssuerProfile ResolveCreditNoteIssuer(SalesCreditNote creditNote) => creditNote.Status == SalesCreditNoteStatus.Posted
