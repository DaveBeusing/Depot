...
public sealed class ItemCostCalculationService
{
	...
	{
		token.ThrowIfCancellationRequested();if(itemId<=0)throw new ArgumentOutOfRangeException(nameof(itemId));var profile=transaction is null?await _costs.GetProfileAsync(itemId,token):await _costs.GetProfileAsync(transaction,itemId,token);if(profile is null)return Fail(itemId,effectiveDate,"No item cost profile exists. Define a base-cost source and currency before calculating cost.");var currency=profile.Currency.Trim().ToUpperInvariant();if(!string.IsNullOrWhiteSpace(expectedCurrency)&&!string.Equals(currency,expectedCurrency.Trim(),StringComparison.OrdinalIgnoreCase))return Fail(itemId,effectiveDate,$"Cost currency {currency} does not match target currency {expectedCurrency.Trim().ToUpperInvariant()}.",currency,profile.BaseCostSource);
		var baseResolution=await ResolveBaseValueAsync(transaction,profile,effectiveDate,token);if(baseResolution.Error is not null)return Fail(itemId,effectiveDate,baseResolution.Error,currency,profile.BaseCostSource);var baseValue=baseResolution.Value??throw new InvalidOperationException("Base-cost resolution returned no value.");if(baseValue.Amount<0m)return Fail(itemId,effectiveDate,"The selected base cost is invalid.",currency,profile.BaseCostSource);
		...
		var evidence=string.Join('|',new[]{$"p:{profile.Id}:{profile.Version}",$"b:{(int)profile.BaseCostSource}:{baseValue.EvidenceType}:{baseValue.EvidenceId}:{baseValue.Version}"}.Concat(results.Select(c=>$"c:{c.ComponentId}:{c.Version}")));return new ItemCostCalculationResult{ItemId=itemId,IsSuccess=true,BaseCost=baseCost,CalculatedCost=running,Currency=currency,CalculationDate=date,BaseCostSource=profile.BaseCostSource,BaseCostEvidenceType=baseValue.EvidenceType,BaseCostEvidenceId=baseValue.EvidenceId,EvidenceVersion=evidence,Components=results};
	}

	private async Task<BaseCostResolution> ResolveBaseValueAsync(DatabaseTransactionContext? transaction,ItemCostProfile profile,DateTime effectiveDate,CancellationToken token)
	{
		switch(profile.BaseCostSource)
		{
			case ItemCostBaseSource.PreferredSupplierPurchasePrice:
				var preferred=transaction is null?await _costs.GetPreferredBaseValuesAsync(profile.ItemId,token):await _costs.GetPreferredBaseValuesAsync(transaction,profile.ItemId,token);if(preferred.Count==0)return new(null,"No active preferred supplier purchase price is available.");if(preferred.Count!=1)return new(null,"Multiple active preferred supplier purchase prices exist. Resolve supplier master-data ambiguity first.");return new(preferred[0],null);
			case ItemCostBaseSource.LastPurchase:
				var last=transaction is null?await _costs.GetLastPurchaseBaseValueAsync(profile.ItemId,effectiveDate,token):await _costs.GetLastPurchaseBaseValueAsync(transaction,profile.ItemId,effectiveDate,token);return last is null?new(null,"No received purchase exists on or before the calculation date."):new(last,null);
			case ItemCostBaseSource.ManualStandard:
				return profile.ManualStandardCost is decimal manual?new(new ItemCostBaseValue(profile.Id,manual,profile.Version,"ManualStandard"),null):new(null,"Manual Standard is selected but no manual standard cost is configured.");
			case ItemCostBaseSource.InventoryCostReference:
				return profile.InventoryCostReference is decimal inventory?new(new ItemCostBaseValue(profile.Id,inventory,profile.Version,"InventoryCostReference"),null):new(null,"Inventory Cost Reference is selected but no reference value is configured.");
			default: throw new ArgumentOutOfRangeException(nameof(profile.BaseCostSource));
		}
	}
	...
	private static void NormalizeProfile(ItemCostProfile value)
	{
		ArgumentNullException.ThrowIfNull(value);if(value.ItemId<=0)throw new ArgumentOutOfRangeException(nameof(value.ItemId));if(!Enum.IsDefined(value.BaseCostSource))throw new ArgumentOutOfRangeException(nameof(value.BaseCostSource));value.Currency=value.Currency.Trim().ToUpperInvariant();if(value.Currency.Length!=3||!value.Currency.All(char.IsLetter))throw new ArgumentException("Item cost currency must be a three-letter ISO currency code.");if(value.ManualStandardCost is <0m)throw new ArgumentException("Manual standard cost cannot be negative.");if(value.InventoryCostReference is <0m)throw new ArgumentException("Inventory cost reference cannot be negative.");if(value.BaseCostSource==ItemCostBaseSource.ManualStandard&&value.ManualStandardCost is null)throw new ArgumentException("Manual Standard requires a configured manual standard cost.");if(value.BaseCostSource==ItemCostBaseSource.InventoryCostReference&&value.InventoryCostReference is null)throw new ArgumentException("Inventory Cost Reference requires a configured reference value.");
	}
	...
	private sealed record BaseCostResolution(ItemCostBaseValue? Value,string? Error);
}
