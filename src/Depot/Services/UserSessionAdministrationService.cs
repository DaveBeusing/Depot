...
public sealed class UserSessionAdministrationService
{
		...
		CancellationToken cancellationToken)
	{
		...
		return await SavePolicyAsync(
			...
			current.ConcurrentSessionMode,
			current.MaximumConcurrentSessions,
			current.ConcurrentSessionLimitAction,
			current.SessionHistoryRetentionDays,
			expectedVersion,
			cancellationToken);
	}
	...
	public async Task<UserSessionPolicy> SavePolicyAsync(
		int idleTimeoutMinutes,
		int maximumSessionAgeHours,
		ConcurrentSessionMode concurrentMode,
		int maximumConcurrentSessions,
		ConcurrentSessionLimitAction limitAction,
		int sessionHistoryRetentionDays,
		long expectedVersion,
		CancellationToken cancellationToken)
	{
		...
		{
			return await SavePolicyLegacyAsync(
				idleTimeoutMinutes,
				maximumSessionAgeHours,
				concurrentMode,
				maximumConcurrentSessions,
		...
		var persisted = await _transactions.ExecuteAsync(async (transaction, token) =>
		{
			...
			after.ConcurrentSessionMode = concurrentMode;
			after.MaximumConcurrentSessions = maximumConcurrentSessions;
			after.ConcurrentSessionLimitAction = limitAction;
			after.SessionHistoryRetentionDays = sessionHistoryRetentionDays;
			after.UpdatedUtc = _timeProvider.GetUtcNow().UtcDateTime;

			if (!await UserSessionRepository.UpdatePolicyAsync(transaction, after, expectedVersion, token))
		...
		await _securityEvents.NotifyPersistedAsync(persisted.Event, cancellationToken);
	...
	public async Task<bool> TerminateSessionAsync(Guid sessionId, CancellationToken cancellationToken)
	{
		...
		{
			var before = await UserSessionRepository.GetBySessionIdAsync(transaction, sessionId, token)
			...
			if (!await UserSessionRepository.EndAsync(transaction, sessionId, endedUtc, UserSessionEndReason.AdministrativeLogout, token))
				return (Ended: false, Event: (SecurityEvent?)null);
			...
			var securityEvent = _securityEvents.CreateSessionEvent(
				before,
				SecurityEventType.AdministrativeSessionTermination,
				SecurityEventSeverity.Warning,
				"User session terminated administratively",
				null);
			securityEvent.Id = await SecurityEventRepository.CreateAsync(transaction, securityEvent, token);
	...
	public async Task<int> TerminateUserSessionsAsync(long userId, CancellationToken cancellationToken)
	{
		...
		var result = await _transactions.ExecuteAsync(async (transaction, token) =>
		{
			...
			var endedUtc = _timeProvider.GetUtcNow().UtcDateTime;
			var ended = await UserSessionRepository.EndActiveSessionsForUserAsync(
				transaction,
				userId,
				endedUtc,
				UserSessionEndReason.AdministrativeLogout,
				token);
			...
			foreach (var session in openSessions)
			{
				...
				var securityEvent = _securityEvents.CreateSessionEvent(
					...
					SecurityEventType.AdministrativeSessionTermination,
					SecurityEventSeverity.Warning,
					"User session terminated by bulk administrative action",
					null);
				securityEvent.Id = await SecurityEventRepository.CreateAsync(transaction, securityEvent, token);
	...
	private async Task<UserSessionPolicy> SavePolicyLegacyAsync(
		int idle,
		int maxAge,
		ConcurrentSessionMode mode,
		int maximum,
		ConcurrentSessionLimitAction action,
		int retention,
		long expectedVersion,
		CancellationToken token)
	{
		var before = await _repository.GetPolicyAsync(token);
		if (before.Version != expectedVersion) throw new ConcurrencyConflictException("user session policy");
		var after = CopyPolicy(before);
		after.IdleTimeoutMinutes = idle;
		after.MaximumSessionAgeHours = maxAge;
		after.ConcurrentSessionMode = mode;
		after.MaximumConcurrentSessions = maximum;
		after.ConcurrentSessionLimitAction = action;
		after.SessionHistoryRetentionDays = retention;
		after.UpdatedUtc = _timeProvider.GetUtcNow().UtcDateTime;
		if (!await _repository.UpdatePolicyAsync(after, expectedVersion, token)) throw new ConcurrencyConflictException("user session policy");
		after.Version = expectedVersion + 1;
		await _repository.ExpireSessionsByPolicyAsync(after.UpdatedUtc, after, token);
		if (_audit is not null) await _audit.RecordActionAsync(after.Id, "UpdateSessionPolicy", before, after, token);
		if (_securityEvents is not null) await _securityEvents.RecordPolicyChangedAsync(before, after, token);
		return after;
	}

	private async Task<bool> TerminateSessionLegacyAsync(Guid sessionId, CancellationToken token)
	{
		var before = await _repository.GetBySessionIdAsync(sessionId, token) ?? throw new InvalidOperationException("The session was not found.");
		if (before.EndedUtc is not null) return false;
		var endedUtc = _timeProvider.GetUtcNow().UtcDateTime;
		if (!await _repository.EndAsync(sessionId, endedUtc, UserSessionEndReason.AdministrativeLogout, token)) return false;
		var after = EndedCopy(before, endedUtc, UserSessionEndReason.AdministrativeLogout);
		if (_audit is not null) await _audit.RecordActionAsync(before.Id, "AdministrativeLogout", before, after, token);
		if (_securityEvents is not null)
			await _securityEvents.RecordSessionEventAsync(before, SecurityEventType.AdministrativeSessionTermination, SecurityEventSeverity.Warning, "User session terminated administratively", null, token);
		return true;
	}

	private async Task<int> TerminateUserSessionsLegacyAsync(long userId, CancellationToken token)
	{
		var openSessions = await _repository.GetOpenSessionsForUserAsync(userId, token);
		if (openSessions.Count == 0) return 0;
		var endedUtc = _timeProvider.GetUtcNow().UtcDateTime;
		var ended = await _repository.EndActiveSessionsForUserAsync(userId, endedUtc, UserSessionEndReason.AdministrativeLogout, token);
		foreach (var session in openSessions)
		{
			var after = EndedCopy(session, endedUtc, UserSessionEndReason.AdministrativeLogout);
			if (_audit is not null) await _audit.RecordActionAsync(session.Id, "AdministrativeLogoutAll", session, after, token);
			if (_securityEvents is not null)
				await _securityEvents.RecordSessionEventAsync(session, SecurityEventType.AdministrativeSessionTermination, SecurityEventSeverity.Warning, "User session terminated by bulk administrative action", null, token);
		}
		return ended;
	}
	...
	private static void ValidatePolicy(
		...
		int retention)
	{
		if (idle is < UserSessionPolicy.MinimumIdleTimeoutMinutes or > UserSessionPolicy.MaximumIdleTimeoutMinutes)
			throw new ArgumentOutOfRangeException(nameof(idle));
		if (maxAge is < UserSessionPolicy.MinimumMaximumSessionAgeHours or > UserSessionPolicy.MaximumMaximumSessionAgeHours)
			throw new ArgumentOutOfRangeException(nameof(maxAge));
		if (!Enum.IsDefined(mode)) throw new ArgumentOutOfRangeException(nameof(mode));
		if (maximum is < UserSessionPolicy.MinimumConcurrentSessions or > UserSessionPolicy.MaximumAllowedConcurrentSessions)
			throw new ArgumentOutOfRangeException(nameof(maximum));
		if (!Enum.IsDefined(action)) throw new ArgumentOutOfRangeException(nameof(action));
		if (retention is < UserSessionPolicy.MinimumSessionHistoryRetentionDays or > UserSessionPolicy.MaximumSessionHistoryRetentionDays)
