...
public sealed class StockMovementReversalService
{
	...
	public StockMovementReversalService(IDatabaseTransactionRunner transactions, InventoryRepository inventories, StockMovementRepository movements, ReasonCodeRepository reasonCodes, AuditRepository auditEntries, AuditService audit)
	{
	...
	public async Task<StockMovement> ReverseWithdrawalAsync(long movementId, long reasonCodeId, string reversalReason, CancellationToken cancellationToken = default)
	{
		...
		var reversals = await _transactions.ExecuteAsync(
			async (transaction, token) =>
			{
				var original = await _movements.GetByIdAsync(transaction, movementId, token)
					?? throw new InvalidOperationException("The stock movement was not found.");
				if (original.MovementType != StockMovementType.Withdrawal)
				{
					throw new InvalidOperationException("Only a material withdrawal can be reversed independently.");
				}
	...
	public async Task<IReadOnlyList<StockMovement>> CreateReversalsAsync(DatabaseTransactionContext transaction, IReadOnlyList<StockMovement> originals, long reasonCodeId, string reversalReason, long userId, CancellationToken cancellationToken)
	{
		...
		if (originals.Select(movement => movement.Id).Distinct().Count() != originals.Count)
		{
			throw new InvalidOperationException("A stock movement can occur only once in a reversal.");
		}
		if (originals.Any(movement => movement.ReversalOfMovementId is not null || movement.MovementType == StockMovementType.Reversal))
		{
			throw new InvalidOperationException("A reversal movement cannot be reversed again.");
		}

		var reasonCode = await _reasonCodes.GetByIdAsync(transaction, reasonCodeId, cancellationToken)
			?? throw new InvalidOperationException("The reversal reason code was not found.");
		...
		var currentQuantities = (await _movements.GetCurrentQuantitiesAsync(transaction, inventoryIds, cancellationToken))
			.ToDictionary(quantity => quantity.InventoryId, quantity => quantity.Quantity);
		var reversalByInventory = originals.GroupBy(movement => movement.InventoryId)
			.ToDictionary(group => group.Key, group => group.Sum(movement => -(long)movement.Quantity));
		if (reversalByInventory.Any(reversal => currentQuantities.GetValueOrDefault(reversal.Key) + reversal.Value < 0))
		{
			throw new InvalidOperationException("The reversal would create a negative stock quantity.");
		}
		...
		foreach (var original in originals.OrderBy(movement => movement.InventoryId).ThenBy(movement => movement.Id))
		{
			var reversal = new StockMovement
			{
				...
				TimestampUtc = reversedAtUtc,
				Quantity = checked(-original.Quantity),
	...
	public long RequireUser() => _audit.CurrentUserId
		?? throw new InvalidOperationException("A signed-in user is required to reverse stock movements.");
