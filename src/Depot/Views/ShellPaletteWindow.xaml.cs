...
public partial class ShellPaletteWindow : Window
{
	...
	private IReadOnlyList<ShellPaletteEntry> BuildCommandEntries(string query)
	{
		...
		if (GetSalesViewModel() is { } sales)
		{
			AddWorkflowAction(entries, "Sales", "Customers", "New Customer", "Create a customer account", CustomerIcon, async () => { if (await NavigateModulePageAsync("Sales", "Customers")) sales.NewCustomerCommand.Execute(null); });
			AddWorkflowAction(entries, "Sales", "Sales Orders", "New Sales Order", "Create a customer sales order", SalesOrderIcon, async () => { if (await NavigateModulePageAsync("Sales", "Sales Orders")) sales.NewOrderCommand.Execute(null); });
			AddWorkflowAction(entries, "Sales", "Shipping", "Create Shipment", "Open shipping for released customer orders", ShipmentIcon, async () => { await NavigateModulePageAsync("Sales", "Shipping"); });
			AddWorkflowAction(entries, "Sales", "Invoices", "Create Sales Invoice", "Create an invoice from a posted shipment", InvoiceIcon, async () => { await NavigateModulePageAsync("Sales", "Invoices"); });
		}
	...
	private async Task RefreshQuickOpenAsync(string query)
	{
		...
		{
			var entries = _recentEntries.Take(8).ToList(); entries.AddRange(navigationEntries); ApplyEntries(entries);
			StatusText.Text = _recentEntries.Count > 0 ? "Recent records and workspaces" : "Workspaces · type to search records"; return;
		}
		...
		try
		{
			...
			_viewModel.ProcurementViewModel.SearchText = oldOrderSearch; _viewModel.ProcurementViewModel.SupplierSearchText = oldSupplierSearch; _viewModel.ProcurementViewModel.Section = oldSection;

			if (GetSalesViewModel() is { } sales)
			{
				foreach (var result in await sales.QuickSearchAsync(text, token))
				{
					var captured = result;
					var (group, type, icon, page, key) = captured.Kind switch
					{
						SalesQuickOpenKind.Customer => ("Customers", "CUSTOMER", CustomerIcon, "Customers", $"customer:{captured.Id}"),
						SalesQuickOpenKind.SalesOrder => ("Sales Orders", "SO", SalesOrderIcon, "Sales Orders", $"sales-order:{captured.Id}"),
						SalesQuickOpenKind.Shipment => ("Shipments", "SHIPMENT", ShipmentIcon, "Shipping", $"shipment:{captured.Id}"),
						_ => ("Sales Invoices", "INVOICE", InvoiceIcon, "Invoices", $"invoice:{captured.Id}")
					};
					entries.Add(new(captured.Title, captured.Subtitle, group, type, icon, async () => { if (!await NavigateModulePageAsync("Sales", page)) return; await sales.OpenQuickItemAsync(captured); }, key));
				}
			}
	...
	private async Task OpenSupplierAsync(Supplier supplier, string query)
	{
		...
		if (_viewModel.AdministrationViewModel.CurrentViewModel is SupplierViewModel suppliers) { suppliers.SearchText = query; await suppliers.LoadAsync(); suppliers.SelectedSupplier = suppliers.Suppliers.FirstOrDefault(candidate => candidate.Id == supplier.Id); }
	}

	private SalesViewModel? GetSalesViewModel()
	{
		var item = _viewModel.NavigationItems.FirstOrDefault(candidate => candidate.Name == "Sales");
		if (item?.Content is not ShellModuleViewModel module) return null;
		return module.Pages.Select(page => page.Content).OfType<SalesViewModel>().FirstOrDefault();
	}

	private async Task<bool> NavigateTopLevelAsync(string name) { var item = _viewModel.NavigationItems.FirstOrDefault(candidate => candidate.Name == name); if (item is null) return false; await _viewModel.NavigateAsync(item); return ReferenceEquals(_viewModel.SelectedNavigationItem, item); }
	private async Task<bool> NavigateModulePageAsync(string moduleName, string pageName) { var item = _viewModel.NavigationItems.FirstOrDefault(candidate => candidate.Name == moduleName); if (item is null || item.Content is not ShellModuleViewModel module) return false; var page = module.Pages.FirstOrDefault(candidate => candidate.Name == pageName); if (page is null || !module.SetSelectedPage(page)) return false; await _viewModel.NavigateAsync(item); return ReferenceEquals(_viewModel.SelectedNavigationItem, item) && ReferenceEquals(module.SelectedPage, page); }
	private bool HasModulePage(string moduleName, string pageName) { var item = _viewModel.NavigationItems.FirstOrDefault(candidate => candidate.Name == moduleName); return item?.Content is ShellModuleViewModel module && module.Pages.Any(candidate => candidate.Name == pageName); }
	private void Remember(ShellPaletteEntry entry) { if (entry.RecordKey is null) return; for (var index = _recentEntries.Count - 1; index >= 0; index--) if (string.Equals(_recentEntries[index].RecordKey, entry.RecordKey, StringComparison.Ordinal)) _recentEntries.RemoveAt(index); _recentEntries.Insert(0, entry with { Group = "Recent" }); while (_recentEntries.Count > 8) _recentEntries.RemoveAt(_recentEntries.Count - 1); }
	private static List<ShellPaletteEntry> Filter(IEnumerable<ShellPaletteEntry> entries, string query) { if (string.IsNullOrWhiteSpace(query)) return entries.ToList(); var terms = query.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries); return entries.Where(entry => terms.All(term => $"{entry.Title} {entry.Subtitle} {entry.Group} {entry.TypeLabel}".Contains(term, StringComparison.OrdinalIgnoreCase))).ToList(); }
	private void ApplyEntries(IReadOnlyList<ShellPaletteEntry> entries) { _updating = true; var view = new ListCollectionView(entries.ToList()); view.GroupDescriptions.Add(new PropertyGroupDescription(nameof(ShellPaletteEntry.Group))); ResultsList.ItemsSource = view; ResultsList.SelectedIndex = entries.Count > 0 ? 0 : -1; if (_mode == ShellPaletteMode.Commands) StatusText.Text = $"{entries.Count:N0} commands"; _updating = false; }
}
