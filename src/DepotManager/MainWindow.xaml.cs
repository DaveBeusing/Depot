...
public partial class MainWindow : Window
{
	...
	public MainWindow()
	{
		...
		if (!_mutex.Acquired) { MessageBox.Show("Another Depot Manager instance is already changing this installation.", "Depot Manager"); Close(); return; }
		Loaded += (_, _) => RefreshStatus();
	}

	private void RefreshStatus()
	{
		var service = Installation; StatusText.Text = service.InstalledVersion is { } version ? $"Installed version: {version}" : "Depot is not installed at this location.";
	}
	...
	private async Task InstallOrUpdateAsync(bool existingRequired, bool repair)
	{
		try
		{
			SetBusy(true); var service = Installation; var installed = service.InstalledVersion;
			if (existingRequired && installed is null) throw new InvalidOperationException("No existing Depot installation was found.");
			var release = await new GitHubReleaseClient(_http).GetLatestAsync(_operation!.Token);
			if (!repair && installed is not null && !VersionRules.IsUpdate(installed, release.Version)) { Log("The installed Depot version is already current."); return; }
			var temp = Path.Combine(Path.GetTempPath(), $"Depot-{release.Version}-{Guid.NewGuid():N}.exe");
			try { await new GitHubReleaseClient(_http).DownloadAsync(release, temp, new Progress<int>(v => Progress.Value = v), _operation.Token); service.Deploy(temp, release.Version, installed is not null); service.CopyManagerToInstallLocation(); service.RegisterInstalledApp(release.Version); }
			finally { if (File.Exists(temp)) File.Delete(temp); }
			RefreshStatus(); Wizard.SelectedIndex = installed is null ? 1 : 0; Log(repair ? "Repair completed." : installed is null ? "Depot installed. Configure the database and administrator before first start." : "Update completed.");
		}
	...
	private async void Provision_Click(object sender, RoutedEventArgs e)
	{
		try
		{
			...
			SetBusy(true); await InvokeDepotManagerModeAsync("--manager-provision", true); var service = Installation; SummaryText.Text = $"Depot successfully installed\n\nVersion: {service.InstalledVersion}\nDatabase: {SelectedProviderName()}\nAdministrator: {AdminEmailBox.Text.Trim()}"; Wizard.SelectedIndex = 3; Log("Database and initial administrator provisioned successfully.");
		}
		catch (Exception ex) { ShowError(ex); }
		finally { SetBusy(false); }
	}
	...
	{
		var provider = ProviderBox.SelectedIndex; int.TryParse(PortBox.Text, out var port);
		return new { Provider = provider == 0 ? 0 : provider == 1 ? 1 : 2, LocalDatabasePath = SqlitePathBox.Text.Trim(), SqlServerHost = provider == 1 ? HostBox.Text.Trim() : string.Empty, SqlServerPort = provider == 1 ? (port == 0 ? 1433 : port) : 1433, SqlServerDatabase = provider == 1 ? DatabaseBox.Text.Trim() : string.Empty, SqlServerUserName = provider == 1 ? UserBox.Text.Trim() : string.Empty, SqlServerPassword = provider == 1 ? DatabasePasswordBox.Password : string.Empty, EncryptSqlServerConnection = true, TrustSqlServerCertificate = false, MySqlHost = provider == 2 ? HostBox.Text.Trim() : string.Empty, MySqlPort = provider == 2 ? (port == 0 ? 3306 : port) : 3306, MySqlDatabase = provider == 2 ? DatabaseBox.Text.Trim() : string.Empty, MySqlUserName = provider == 2 ? UserBox.Text.Trim() : string.Empty, MySqlPassword = provider == 2 ? DatabasePasswordBox.Password : string.Empty, UseMySqlTls = true, AutomaticBackupsEnabled = false, BackupDirectory = "Backups", BackupIntervalDays = 1 };
	}

	private void Provider_Changed(object sender, SelectionChangedEventArgs e) { if (PortBox is null) return; PortBox.Text = ProviderBox.SelectedIndex == 2 ? "3306" : "1433"; }
	...
	private void Uninstall_Click(object sender, RoutedEventArgs e) { try { if (MessageBox.Show("Remove Depot application files? Database and business data will not be deleted.", "Depot Manager", MessageBoxButton.YesNo, MessageBoxImage.Warning) != MessageBoxResult.Yes) return; Installation.Uninstall(false); RefreshStatus(); Log("Depot application files removed. Configuration and data were preserved."); } catch (Exception ex) { ShowError(ex); } }
	...
	private void Log(string text) { DetailText.Text = text; try { var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Depot", "Logs"); Directory.CreateDirectory(dir); File.AppendAllText(Path.Combine(dir, "DepotManager.log"), $"{DateTimeOffset.Now:O} {text}{Environment.NewLine}"); } catch { } }
